...
class AuthServiceTest {
  ...
  void testVerifyOtpAndLogin_Success_NewUser() {
    ...
    when(otpRepository.findTopByEmailOrderByExpiryTimeDesc(testEmail))
    ...
    // Act
    Optional<User> result = authService.verifyOtpAndLogin(testEmail, validOtp);
  ...
  void testVerifyOtpAndLogin_Failure_Expired() {
    ...
    Otp otpEntity =
        new Otp(testEmail, Otp.hash(expiredOtp), LocalDateTime.now().minusMinutes(1)); // Expired

    when(otpRepository.findTopByEmailOrderByExpiryTimeDesc(testEmail))
        .thenReturn(Optional.of(otpEntity));

    // Act
    Optional<User> result = authService.verifyOtpAndLogin(testEmail, expiredOtp);
  ...
  void testVerifyOtpAndLogin_Failure_InvalidCode() {
    ...
    // Act
    Optional<User> result = authService.verifyOtpAndLogin(testEmail, inputOtp);
