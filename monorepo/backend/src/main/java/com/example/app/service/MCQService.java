...
public class MCQService {
    ...
    public Optional<MCQResponse> getMCQByEpisodeId(UUID episodeId) {
        ...
        List<MCQOptionDTO> optionDTOs = options.stream()
                .map(option -> new MCQOptionDTO(option.getId(), option.getOptionText()))
                .collect(Collectors.toList());

        MCQResponse response = new MCQResponse(mcq.getId(), mcq.getQuestion(), optionDTOs, mcq.getRefresherVideoUrl());
    ...
    public boolean validateAnswer(UUID mcqId, UUID selectedOptionId) {
        ...
        MCQOption selectedOption = mcqOptionRepository
